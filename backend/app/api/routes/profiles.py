...
from app.schemas.profiles import (
    ...
    PatientProfileRead, DoctorProfileRead
)
...
from app.crud.auth import update_user
import logging
import math
import uuid
...
async def get_current_user_profile(request: Request):
    ...
    try:
        ...
        if user_data.is_patient and profile_data:
            response_data["patient_profile"] = PatientProfileRead.model_validate(profile_data)
        elif user_data.is_doctor and profile_data:
            response_data["doctor_profile"] = DoctorProfileRead.model_validate(profile_data)
    ...
    try:
        ...
        if current_user.is_patient:
            ...
            if patient_fields:
                ...
                updated_profile = update_patient_profile(current_user.id, patient_update)
        ...
        if updated_user.is_patient and updated_profile:
            response_data["patient_profile"] = PatientProfileRead.model_validate(updated_profile)
        elif updated_user.is_doctor and updated_profile:
            response_data["doctor_profile"] = DoctorProfileRead.model_validate(updated_profile)
...
async def search_doctors_endpoint(
    ...
    hospital_affiliation: str = Query(None, description="Filter by hospital affiliation"),
    ...
    try:
        ...
        doctor_reads = [DoctorProfileRead.model_validate(doctor) for doctor in doctors]
        
        return DoctorSearchResponse(
            doctors=doctor_reads,
